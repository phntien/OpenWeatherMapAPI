...
public class TC_API_0002_Get_current_weather_data_with_INVALID_CityName extends BaseTest {

    @Test(dataProvider = "excelData", dataProviderClass = DataProviderFactory.class)
    public void TC_API_0002_Get_current_weather_data_with_INVALID_CityName(Map<String, String> data) {

        String cityName = data.get("InvalidCityName");
        String endPoint = Constants.DEFAULT_API_URL + cityName + Constants.DEFAULT_API_KEY;

        logger.info("1. Send Get request to the server with city = " + cityName);
